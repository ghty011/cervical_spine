...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    is_training = isinstance(path, list)
    with torch_distributed_zero_first(rank):
        if is_training:
            # training
            dataset = LoadImagesAndLabelsTraining(path, imgsz, batch_size,
...
class LoadImagesAndLabelsTraining(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
        ...
        self.neg_count = 0
        #self.albumentations = Albumentations() if augment else None
        # print("path list count : ", path)
        try:
            f = []  # image files
            for path_index, p in enumerate(path) if isinstance(path, list) else [path]:
                p = Path(p)  # os-agnostic
                if p.is_dir():  # dir
                    f += glob.glob(str(p / '**' / '*.*'), recursive=True)
                    # f = list(p.rglob('**/*.*'))  # pathlib
                    ...
                    with open(p, 'r') as t:
                        ...
                        if path_index == 0:
                            self.pos_count += len(t)
                        else:
                            self.neg_count += len(t)
        ...
        print("positive and negative count", self.pos_count, self.neg_count)
        # Check cache
        self.label_files = img2label_paths(self.img_files)  # labels
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_suffix('.cache')  # cached labels
        if cache_path.is_file():
    ...
    def get_random_index(self, index):
        # 여기서 2 는 위에 1 + 1 = 2다 , 1:2 의 비례로 넣으려면 여기서 3으로 해라
        total = 2
        if index % total == 0:
            # get positive
            return index // total
            ...
            # get negative
            index = np.random.choice(self.neg_count)
            return self.pos_count + index
